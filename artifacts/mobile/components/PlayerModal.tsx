...
// Ionicons-only repeat icon — avoids MaterialCommunityIcons CJK fallback on Android
function RepeatIcon({ mode, color }: { mode: 'none' | 'all' | 'one'; color: string }) {
  return (
    <View style={repeatStyles.wrap}>
      <Ionicons
        name="repeat"
        size={24}
        color={mode === 'none' ? color : '#A855F7'}
      />
      {mode === 'one' && (
        <View style={repeatStyles.badge}>
          <Text style={repeatStyles.badgeText}>1</Text>
        </View>
      )}
      {mode === 'none' && (
        // Strikethrough-style dim bar across the repeat icon
        <View style={repeatStyles.offBar} />
      )}
    </View>
  );
}
...
const repeatStyles = StyleSheet.create({
  wrap: { position: 'relative', width: 30, height: 30, alignItems: 'center', justifyContent: 'center' },
  badge: {
    position: 'absolute',
    top: -2,
    right: -2,
  ...
  badgeText: { color: '#fff', fontSize: 8, fontWeight: '700', lineHeight: 12 },
  offBar: {
    position: 'absolute',
    width: 26,
    height: 1.5,
...
export function PlayerModal() {
  ...
  const skipNextStyle = useAnimatedStyle(() => ({ transform: [{ scale: skipNextScale.value }] }));
  const skipPrevStyle = useAnimatedStyle(() => ({ transform: [{ scale: skipPrevScale.value }] }));

  // ── Swipe down to dismiss ──────────────────────────────────────────────────
  const swipePan = PanResponder.create({
    onStartShouldSetPanResponder: () => false,
    onMoveShouldSetPanResponder: (_, g) => g.dy > 15 && Math.abs(g.dx) < Math.abs(g.dy),
    onPanResponderRelease: (_, g) => { if (g.dy > 80) setShowPlayer(false); },
  });
  ...
  return (
    ...
    >
      <View style={styles.root} {...swipePan.panHandlers}>
          ...
          >
            ...
            <View
              style={[
                ...
                { left: `${Math.min(progress * 100, 100)}%`, backgroundColor: colors.lavender },
              ]}
            />
          </View>
          <View style={styles.timeRow}>
            <Text style={[styles.timeText, { color: colors.mutedForeground }]}>{formatTime(displayMs)}</Text>
            <Text style={[styles.timeText, { color: colors.mutedForeground }]}>{formatTime(durationMs)}</Text>
          </View>
        </View>
        ...
        <View style={styles.controlsOuter}>
          ...
          <View style={styles.controls}>
            ...
            {/* Repeat — pure Ionicons, no MaterialCommunityIcons */}
            <TouchableOpacity
              hitSlop={{ top: 12, bottom: 12, left: 12, right: 12 }}
              onPress={toggleRepeat}
            >
              <RepeatIcon mode={repeat} color={colors.mutedForeground} />
            </TouchableOpacity>
...
const styles = StyleSheet.create({
  root: { flex: 1 },
  glowOrb: {
    position: 'absolute', top: 0, left: 0, right: 0, height: '65%', zIndex: 0,
  },
  header: {
    flexDirection: 'row',
    alignItems: 'center',
